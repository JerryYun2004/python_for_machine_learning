...
if __name__ == "__main__":
    ...
    # Keep only diamonds and clubs
    df1 = df1[df1["class"].str.contains("diamonds|clubs")]
    df2 = df2[df2["class"].str.contains("diamonds|clubs")]

    # Replace full class name with just suit name
    df1["class"] = df1["class"].apply(lambda x: "diamonds" if "diamonds" in x else "clubs")
    df2["class"] = df2["class"].apply(lambda x: "diamonds" if "diamonds" in x else "clubs")
    ...
    label_encoder.fit(df["class"])  # Now only two classes: diamonds, clubs

    with open("label_encoder_red.pkl", "wb") as f:
    ...
    model = models.Sequential([
        ...
        layers.Dropout(0.3),
        layers.Dense(2, activation='softmax')  # Only two classes now: diamonds vs clubs
    ])
    ...
    model.save("model_CNN_diamonds_vs_clubs.keras")
