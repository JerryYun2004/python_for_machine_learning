...
class CardDatasetGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        for _, row in batch_df.iterrows():
            ...
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB) if img.shape[2] == 3 else cv2.cvtColor(img, cv2.COLOR_GRAY2RGB)
...
if __name__ == "__main__":
    ...
    # Extract rank from class name (e.g., "king" from "king of diamonds")
    df["rank"] = df["class"].apply(lambda x: x.split(" of ")[0].strip().lower())

    label_encoder = LabelEncoder()
    label_encoder.fit(df["rank"])
    df["class"] = df["rank"]  # overwrite for compatibility with generator

    # Save encoder
    with open("label_encoder_rank.pkl", "wb") as f:
    ...
    # Generators
    train_gen = CardDatasetGenerator(train_df, label_encoder, batch_size=BATCH_SIZE, image_size=TARGET_SIZE)
    val_gen = CardDatasetGenerator(val_df, label_encoder, batch_size=BATCH_SIZE, image_size=TARGET_SIZE, shuffle=False)
    ...
    model.save("model_CNN_rank.keras")
