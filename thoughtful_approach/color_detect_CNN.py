...
class CardDatasetGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        for _, row in batch_df.iterrows():
            filename, label, root = row.get("filename"), row.get("color"), row.get("root")
...
if __name__ == "__main__":
    ...
    def classify_color(card):
        card = str(card).lower()
        if 'diamond' in card or 'heart' in card:
            return 'red'
        elif 'club' in card or 'spade' in card:
            return 'black'
        else:
            return np.nan

    df['color'] = df['class'].apply(classify_color)
    df.dropna(subset=['color'], inplace=True)

    label_encoder = LabelEncoder()
    label_encoder.fit(df["color"])

    with open("label_encoder_red_black.pkl", "wb") as f:
    ...
    model.save("model_CNN_red_black.keras")